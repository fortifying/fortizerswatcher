...
from fortizers import dispatcher, LOGGER, spamcheck, OWNER_ID, SUDO_USERS
from fortizers.modules.helper_funcs.chat_status import user_not_admin, user_admin
from fortizers.modules.log_channel import loggable
from fortizers.modules.sql import reporting_sql as sql
...
def report_alt(update, context) -> str:
    ...
    if chat and message.reply_to_message and sql.chat_should_report(chat.id):
        ...
        context.bot.send_message(chat.id,
                                 tl(update.effective_message, "⚠️ {} <b>telah di laporkan ke admin!</b>{}").format(
                                     mention_html(reported_user.id, reported_user.first_name),
                                     "".join(all_admins)), parse_mode=ParseMode.HTML,
...
def button(update, context):
    ...
    if splitter[1] == "1":
        keyboard = [
            [InlineKeyboardButton(tl(update.effective_message, "Ya"),
                                  callback_data="ak_1+y|{}={}".format(report_chat, report_target)),
             InlineKeyboardButton(tl(update.effective_message, "Tidak"),
                                  callback_data="ak_1+n|{}={}".format(report_chat, report_target))]
        ]
        reply_markup = InlineKeyboardMarkup(keyboard)
        context.bot.edit_message_text(
            text=msg + tl(update.effective_message, "\n\nApakah anda yakin ingin menendang {}?").format(
                userinfo.get('name')),
        ...
        keyboard = [
            [InlineKeyboardButton(tl(update.effective_message, "Ya"),
                                  callback_data="ak_2+y|{}={}".format(report_chat, report_target)),
             InlineKeyboardButton(tl(update.effective_message, "Tidak"),
                                  callback_data="ak_2+n|{}={}".format(report_chat, report_target))]
        ]
        reply_markup = InlineKeyboardMarkup(keyboard)
        context.bot.edit_message_text(
            text=msg + tl(update.effective_message, "\n\nApakah anda yakin ingin banned {}?").format(
                userinfo.get('name')),
            chat_id=query.message.chat_id,
            message_id=query.message.message_id, parse_mode=ParseMode.HTML,
            reply_markup=reply_markup)
        ...
        keyboard = [
            [InlineKeyboardButton(tl(update.effective_message, "Ya"),
                                  callback_data="ak_3+y|{}={}".format(report_chat, report_target)),
             InlineKeyboardButton(tl(update.effective_message, "Tidak"),
                                  callback_data="ak_3+n|{}={}".format(report_chat, report_target))]
        ]
        reply_markup = InlineKeyboardMarkup(keyboard)
