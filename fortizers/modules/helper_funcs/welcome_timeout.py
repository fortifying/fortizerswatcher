...
from fortizers import dispatcher, updater, spamcheck
...
def timeout_mode(update, context):
    ...
    if args:
        if args[0].lower() == 'kick' or args[0].lower() == 'tendang' or args[0].lower() == 'leave':
            ...
            return
        if conn:
            text = tl(update.effective_message, "Mode timeout diubah, Pengguna akan di `{}` pada *{}*!").format(
        ...
        if timeout_mode == 1:
            ...
            settypeblacklist = tl(update.effective_message, "banned")
        if conn:
            text = tl(update.effective_message, "Mode timeout saat ini disetel ke *{}* pada *{}*.").format(
                settypeblacklist, chat_name)
        else:
