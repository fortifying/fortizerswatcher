...
from fortizers import dispatcher, SUDO_USERS, OWNER_USERNAME, OWNER_ID
from fortizers.modules.helper_funcs.extraction import extract_user
from fortizers.modules.helper_funcs.chat_status import bot_admin


@bot_admin
@run_async
def addsudo(update, context: List[str]):
    args = context.args
...
@bot_admin
@run_async
def rsudo(update, context: List[str]):
    args = context.args
    message = update.effective_message
    user_id = extract_user(message, args)
