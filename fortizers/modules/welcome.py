...
from fortizers import dispatcher, OWNER_ID, LOGGER, spamcheck, IS_DEBUG, SUDO_USERS, MESSAGE_DUMP
try:
	from fortizers import SPAMWATCH_TOKEN
except:
	pass
from fortizers.modules.helper_funcs.chat_status import user_admin, is_user_ban_protected, bot_can_restrict
from fortizers.modules.helper_funcs.misc import build_keyboard_parser, revert_buttons
...
from fortizers.modules.helper_funcs.welcome_timeout import welcome_timeout
...
def check_sw(bot: Bot, user_id, user, message):
	...
	try:
		...
		context.bot.sendMessage(message.chat.id, "*⚠️ WARNING!*\n{} is a spammer from SpamWatch and has been added to fedban list of *FortizersWatcherCircle*!\n\nIt's recommended to banned him/her!".format(mention_markdown(user_id, user.first_name)), parse_mode="markdown", disable_web_page_preview=True)
	if is_success:
		context.bot.sendMessage(message.chat.id, "{} has been banned and added to fedban list of *FortizersWatcherCircle*!\nReason: {}.".format(mention_markdown(user_id, user.first_name), json.get('reason') if json.get('reason') else "Unknown reason"), parse_mode="markdown", disable_web_page_preview=True)
	fed_id = fedsql.get_fed_info("FortizersWatcherCircle")
	if fed_id:
		x = fedsql.fban_user("FortizersWatcherCircle", user_id, user.first_name, user.last_name, user.username, json.get('reason') if json.get('reason') else "Unknown reason", int(time.time()))
		if not x:
			LOGGER.warning("Cannot fban spammer user!")
			return
