...
from fortizers import dispatcher, spamcheck, LOGGER
from fortizers.modules.disable import DisableAbleCommandHandler
from fortizers.modules.helper_funcs.chat_status import user_admin_no_reply, user_admin

from fortizers.modules.sql import languages_sql as sql
...
for x in os.listdir('fortizers/modules/langs'):
	if os.path.isdir('fortizers/modules/langs/'+x):
		continue
	x = x.replace('.py', '')
	LOADED_LANGS_ID.append(x)
	imported_langs = importlib.import_module("fortizers.modules.langs." + x)
...
def set_language(update, context):
	...
	loaded_langs.append([InlineKeyboardButton("Translate", url="https://github.com/fortifying/fortizerswatcher/blob/master/TRANSLATION.md")])
