...
from fortizers.modules.helper_funcs.handlers import CMD_STARTERS
from fortizers.modules.helper_funcs.misc import is_module_loaded, send_to_list
	...
	if args:
		if args[0] == 'json':
			...
			with BytesIO(str.encode(backups)) as output:
				...
				update.effective_message.reply_document(document=output, filename="fortizers_fbanned_users.json",
			...
			with BytesIO(str.encode(backups)) as output:
				...
				update.effective_message.reply_document(document=output, filename="fortizers_fbanned_users.csv",
													caption=tl(update.effective_message, "Total {} pengguna terkena blokir federasi {}.").format(len(getfban), info['fname']))
			return
