...
from fortizers import dispatcher, OWNER_ID, SUDO_USERS, WHITELIST_USERS, TEMPORARY_DATA, LOGGER, spamcheck
from fortizers.modules.helper_funcs.chat_status import is_user_admin
from fortizers.modules.helper_funcs.extraction import extract_user, extract_unt_fedban, extract_user_fban
...
import fortizers.modules.sql.feds_sql as sql
...
def new_fed(update, context):
    ...
    if chat.type != "private":
        send_message(
            update.effective_message, tl(
                update.effective_message, "Buat federasi Anda di PM saya, bukan dalam grup."))
        return
    if len(message.text) == 1:
        send_message(
            update.effective_message, tl(
                update.effective_message, "Tolong tulis nama federasinya!"))
        return
    ...
    if not fednam == '':
        ...
        elif fednam == "fortizers Official":
            fed_id = "fortizersOfficial"
        ...
        if not x:
            send_message(
                ...
                tl(
                    update.effective_message,
                    "Tidak dapat membuat federasi! Tolong hubungi pembuat saya jika masalah masih berlanjut."))
            return
        ...
        try:
            context.bot.send_message(
                TEMPORARY_DATA,
                "Federasi <b>{}</b> telah di buat dengan ID: <pre>{}</pre>".format(
                    fed_name,
                    fed_id),
...
def del_fed(update, context):
    ...
    if chat.type != "private":
        send_message(
            update.effective_message, tl(
                update.effective_message, "Hapus federasi Anda di PM saya, bukan dalam grup."))
        return
    if args:
        is_fed_id = args[0]
        ...
        if getinfo is False:
            send_message(
                update.effective_message, tl(
                    update.effective_message, "Federasi ini tidak di temukan!"))
            return
        ...
        send_message(
            update.effective_message, tl(
                update.effective_message, "Apa yang harus saya hapus?"))
        return
...
def fed_chat(update, context):
    ...
    text = tl(
        update.effective_message,
        "Obrolan ini adalah bagian dari federasi berikut:")
...
def join_fed(update, context):
        ...
        for admin in administrators:
            ...
            if status == "creator":
                if str(admin.user.id) == str(user.id):
                    ...
                    send_message(
                        update.effective_message, tl(
                            update.effective_message, "Hanya pembuat grup yang dapat melakukannya!"))
                    return
    if fed_id:
        send_message(update.effective_message,
                     tl(update.effective_message,
                        "Anda tidak bisa bergabung dua federasi dalam satu obrolan"))
        return
    ...
    if len(args) >= 1:
        ...
        if getfed is False:
            send_message(
                update.effective_message, tl(
                    update.effective_message, "Silakan masukkan id federasi yang valid."))
            return
        ...
        if not x:
            send_message(
                ...
                tl(
                    update.effective_message,
                    "Gagal bergabung dengan federasi! Tolong hubungi pembuat saya jika masalah ini masih berlanjut."))
            return
        ...
        if get_fedlog:
            if eval(get_fedlog):
                context.bot.send_message(
                    get_fedlog, tl(
                        update.effective_message, "Obrolan *{}* telah bergabung ke federasi *{}*").format(
                        chat.title, getfed['fname']), parse_mode="markdown")
 
        send_message(
            update.effective_message, tl(
                update.effective_message, "Obrolan ini telah bergabung dengan federasi {}!").format(
                getfed['fname']))
        ...
        if sql.chat_leave_fed(chat.id) is True:
            ...
            if get_fedlog:
                if eval(get_fedlog):
                    context.bot.send_message(
                        get_fedlog, tl(
                            update.effective_message, "Obrolan *{}* telah keluar ke federasi *{}*").format(
                            chat.title, fed_info['fname']), parse_mode="markdown")
            send_message(
                update.effective_message, tl(
                    update.effective_message, "Obrolan ini telah keluar dari federasi {}!").format(
                    fed_info['fname']))
        else:
...
def user_join_fed(update, context):
    ...
    if is_user_fed_owner(fed_id, user.id):
        ...
        if user_id == get_owner:
            send_message(
                update.effective_message, tl(
                    update.effective_message, "Mengapa Anda mencoba mempromosikan pemilik federasi!?"))
            return
        if getuser:
            send_message(
                update.effective_message,
        ...
        if res:
            send_message(
                update.effective_message, tl(
                    update.effective_message, "💖 Berhasil dinaikan jabatannya!"))
        else:
            send_message(
                update.effective_message, tl(
                    update.effective_message, "Gagal dipromosikan!"))
    else:
        send_message(
            update.effective_message, tl(
                update.effective_message, "Hanya pemilik federasi yang dapat melakukan ini!"))
...
def user_demote_fed(update, context):
    ...
    if is_user_fed_owner(fed_id, user.id):
        ...
        if user_id == context.bot.id:
            send_message(update.effective_message, tl(update.effective_message,
                                                      "Apa yang sedang Anda coba lakukan? Menurunkan saya dari federasi Anda?"))
            return
        ...
        if sql.search_user_in_fed(fed_id, user_id) is False:
            send_message(
                ...
                tl(
                    update.effective_message,
                    "Saya tidak dapat mendemosikan pengguna yang bukan merupakan admin federasi! Jika Anda ingin membuatnya menangis, promosikan dia terlebih dahulu!"))
            return
        ...
        if res is True:
            send_message(
                update.effective_message, tl(
                    update.effective_message, "💔 Dia telah di keluarkan dari federasi anda!"))
        else:
            send_message(
                update.effective_message, tl(
                    update.effective_message, "Saya tidak bisa mengusirnya, Saya tidak berdaya!"))
    else:
        send_message(
            update.effective_message, tl(
                update.effective_message, "Hanya pemilik federasi yang dapat melakukan ini!"))
...
def fed_ban(update, context):
    ...
    if not user_id:
        send_message(
            update.effective_message, tl(
                update.effective_message, "Anda sepertinya tidak merujuk ke pengguna."))
        return
    ...
    if is_user_fed_owner(fed_id, user_id) is True:
        send_message(
            update.effective_message, tl(
                update.effective_message, "Mengapa Anda mencoba fban pemilik federasi?"))
        return
 
    if is_user_fed_admin(fed_id, user_id) is True:
        send_message(update.effective_message,
                     tl(update.effective_message,
                        "Dia adalah admin dari federasi, saya tidak bisa fban dia."))
        return
 
    if user_id is OWNER_ID:
        send_message(update.effective_message, tl(update.effective_message,
                                                  "Aku tidak ingin memblokir tuanku, itu ide yang sangat bodoh!"))
        return
 
    if int(user_id) in SUDO_USERS:
        send_message(update.effective_message, tl(
            update.effective_message, "Saya tidak akan fban pengguna sudo!"))
        return
 
    if int(user_id) in WHITELIST_USERS:
        send_message(update.effective_message,
                     tl(update.effective_message,
                        "Orang ini masuk daftar putih, jadi tidak bisa di fban!"))
        return
    ...
    try:
        ...
        if not str(user_id).isdigit():
            ...
            send_message(
                update.effective_message, tl(
                    update.effective_message, "Itu bukan pengguna!"))
            return
        isvalid = False
    ...
    if fban:
        ...
        starting = tl(
            update.effective_message,
            "Alasan fban di ganti untuk {} pada Federasi <b>{}</b>.").format(
            user_target,
            fed_name)
        send_message(
            update.effective_message,
        ...
        if not temp:
            send_message(
                update.effective_message, tl(
                    update.effective_message, "Gagal mengupdate alasan fedban!"))
            return
        ...
        if not x:
            send_message(
                ...
                tl(
                    update.effective_message,
                    "Gagal melarangan federasi! Jika masalah ini terus terjadi, hubungi pembuat saya."))
            return
            ...
            try:
                ...
                if excp.message in FBAN_ERRORS:
                    try:
                        ...
                        LOGGER.info(
                            "Chat {} has leave fed {} because bot is kicked".format(
                                fedschat, info['fname']))
                        continue
                    ...
                    try:
                        ...
                        if excp.message in FBAN_ERRORS:
                            try:
                                ...
                                LOGGER.info(
                                    "Chat {} has unsub fed {} because bot is kicked".format(
                                        fedschat, info['fname']))
                                continue
                        elif excp.message == "User_id_invalid":
                            break
        ...
        send_message(
            update.effective_message, tl(
                update.effective_message, "Alasan fedban telah di perbarui."))
        return
    ...
    starting = tl(
        update.effective_message,
        "Memulai larangan federasi untuk {} pada Federasi <b>{}</b>.").format(
        user_target,
        fed_name)
        ...
        for fedsid in subscriber:
            ...
            for fedschat in all_fedschat:
                try:
                    ...
                    pass
    send_message(update.effective_message, tl(
        update.effective_message, "Orang ini telah di fbanned."))
...
def unfban(update, context):
    ...
    if not fban:
        send_message(
            update.effective_message, tl(
                update.effective_message, "Pengguna ini tidak di fbanned!"))
        return
    ...
    send_message(
        ...
        tl(
            update.effective_message,
            "Saya akan memberi {} kesempatan kedua dalam federasi ini.").format(user_target),
        parse_mode="HTML")
    ...
    send_message(update.effective_message, tl(
        update.effective_message, "Orang ini telah di un-fbanned."))
...
def set_frules(update, context):
    ...
    if len(args) >= 1:
        ...
        if not x:
            send_message(
                update.effective_message,
                "There is an error while setting federation rules! If you wondered why please ask it in support group!")
            return
        ...
        if get_fedlog:
            if eval(get_fedlog):
                context.bot.send_message(
                    get_fedlog, tl(
                        update.effective_message, "*{}* telah mengganti aturan federasi *{}*").format(
                        user.first_name, getfed['fname']), parse_mode="markdown")
        send_message(
            update.effective_message, tl(
                update.effective_message, "Aturan telah di ganti menjadi:\n{}!").format(rules))
    else:
        send_message(
            update.effective_message, tl(
...
def fed_broadcast(update, context):
    ...
    if args:
        ...
        for chat in chat_list:
            title = tl(
                chat, "*Siaran baru dari Federasi {}*\n").format(fedinfo['fname'])
            try:
        ...
        if failed >= 1:
            send_text += tl(
                update.effective_message,
                "{} grup gagal menerima pesan, mungkin karena meninggalkan federasi.").format(failed)
...
def fed_ban_list(update, context):
    ...
    if len(getfban) == 0:
        send_message(
            update.effective_message, tl(
                update.effective_message, "Tidak ada pengguna yang di fban di federasi {}").format(
    ...
    if args:
        if args[0] == 'json':
            ...
            with BytesIO(str.encode(backups)) as output:
                output.name = "fortizers_fbanned_users.json"
                update.effective_message.reply_document(
                    document=output,
                    filename="fortizers_fbanned_users.json",
                    caption=tl(
                        update.effective_message,
                        "Total {} pengguna terkena blokir federasi {}.").format(
                        len(getfban),
                        info['fname']))
            return
        elif args[0] == 'csv':
            ...
            for users in getfban:
                ...
                backups += "\n"
            with BytesIO(str.encode(backups)) as output:
                output.name = "fortizers_fbanned_users.csv"
                update.effective_message.reply_document(
                    document=output,
                    filename="fortizers_fbanned_users.csv",
                    caption=tl(
                        update.effective_message,
                        "Total {} pengguna terkena blokir federasi {}.").format(
                        len(getfban),
    ...
    for users in getfban:
        ...
        if not getuserinfo:
            text = tl(
                update.effective_message,
                "Tidak ada pengguna yang di fban di federasi {}").format(
                info['fname'])
            break
    ...
    try:
        ...
        with BytesIO(str.encode(cleantext)) as output:
            ...
            update.effective_message.reply_document(
                ...
                caption=tl(
                    update.effective_message,
                    "Berikut adalah daftar pengguna yang saat ini difban pada federasi {}.").format(
                    info['fname']))
...
def fed_chats(update, context):
    ...
    if len(getlist) == 0:
        send_message(
            update.effective_message, tl(
                update.effective_message, "Tidak ada obrolan yang bergabung pada federasi {}").format(
                info['fname']), parse_mode=ParseMode.HTML)
        return
    ...
    try:
        ...
        with BytesIO(str.encode(cleantext)) as output:
            ...
            update.effective_message.reply_document(
                ...
                caption=tl(
                    update.effective_message,
                    "Berikut adalah daftar obrolan yang bergabung federasi {}.").format(
                    info['fname']))
...
def fed_import_bans(update, context):
    ...
    if msg.reply_to_message and msg.reply_to_message.document:
        ...
        if fileformat == 'json':
            ...
            text = tl(
                update.effective_message,
                "Berkas blokir berhasil diimpor. {} orang diblokir.").format(success)
            if failed >= 1:
                text += tl(update.effective_message,
                           " {} gagal di impor.").format(failed)
            get_fedlog = sql.get_fed_log(fed_id)
            if get_fedlog:
                if eval(get_fedlog):
                    teks = tl(
                        update.effective_message,
                        "Federasi *{}* telah berhasil mengimpor data. {} di blokir").format(
                        getfed['fname'],
                        success)
                    if failed >= 1:
            ...
            send_message(
                update.effective_message, tl(
                    update.effective_message, "File tidak di dukung."))
            return
...
def del_fed_button(update, context):
    ...
    if fed_id == 'cancel':
        query.message.edit_text(
            tl(update.effective_message, "Penghapusan federasi dibatalkan"))
        return
    ...
    if user_id and user_id != "error":
        if len(args) == 2 and args[0].isdigit():
            ...
            if not reason:
                send_message(
                    update.effective_message, tl(
                        update.effective_message, "{} belum di larang di federasi ini!").format(user_name))
            else:
                teks = tl(
                    update.effective_message,
        ...
        if not fedinfo:
            send_message(
                update.effective_message, tl(
                    update.effective_message, "Federasi {} tidak di temukan!").format(fed_id))
            return
        ...
        if not reason:
            send_message(
                update.effective_message, tl(
                    update.effective_message, "{} tidak di larang di federasi ini").format(name))
            return
        send_message(update.effective_message,
                     tl(update.effective_message,
                        "{} di larang di federasi ini karena:\n`{}`\n*Di banned pada:* `{}`").format(name,
                                                                                                     reason,
                                                                                                     fbantime),
                     parse_mode="markdown")
...
def set_fed_log(update, context):
    ...
    if args:
        ...
        if setlog:
            send_message(
                update.effective_message, tl(
                    update.effective_message, "Log federasi `{}` telah di setel pada {}").format(
                    fedinfo['fname'], chat.title), parse_mode="markdown")
    else:
        send_message(update.effective_message, tl(
            update.effective_message, "Anda belum memberikan ID federasinya!"))
...
def unset_fed_log(update, context):
    ...
    if args:
        ...
        if setlog:
            send_message(
                update.effective_message, tl(
                    update.effective_message, "Log federasi `{}` telah di cabut pada {}").format(
                    fedinfo['fname'], chat.title), parse_mode="markdown")
    else:
        send_message(update.effective_message, tl(
            update.effective_message, "Anda belum memberikan ID federasinya!"))
        ...
        if subfed:
            ...
            if get_fedlog:
                if int(get_fedlog) != int(chat.id):
                    context.bot.send_message(
                        get_fedlog, tl(
                            update.effective_message, "Federasi `{}` telah mengikuti federasi `{}`").format(
                            fedinfo['fname'], getfed['fname']), parse_mode="markdown")
        else:
            send_message(
                update.effective_message, tl(
                    update.effective_message, "Federasi `{}` sudah mengikuti federasi `{}`.").format(
                    fedinfo['fname'], getfed['fname']), parse_mode="markdown")
    else:
        send_message(update.effective_message, tl(
            update.effective_message, "Anda belum memberikan ID federasinya!"))
...
def unsubs_feds(update, context):
    ...
    if args:
        ...
        if subfed:
            send_message(
                update.effective_message, tl(
                    update.effective_message, "Federasi `{}` sudah tidak mengikuti `{}` lagi.").format(
                    fedinfo['fname'], getfed['fname']), parse_mode="markdown")
            get_fedlog = sql.get_fed_log(args[0])
            if get_fedlog:
                if int(get_fedlog) != int(chat.id):
                    context.bot.send_message(
                        get_fedlog, tl(
                            update.effective_message, "Federasi `{}` sudah tidak mengikuti `{}`").format(
                            fedinfo['fname'], getfed['fname']), parse_mode="markdown")
        else:
            send_message(
                update.effective_message, tl(
                    update.effective_message, "Federasi `{}` tidak mengikuti federasi `{}`.").format(
                    fedinfo['fname'], getfed['fname']), parse_mode="markdown")
    else:
        send_message(update.effective_message, tl(
            update.effective_message, "Anda belum memberikan ID federasinya!"))
...
def get_myfedsubs(update, context):
    ...
    if len(getmy) == 0:
        send_message(
            update.effective_message, tl(
                update.effective_message, "Federasi `{}` tidak mengikuti federasi apapun.").format(
                fedinfo['fname']), parse_mode="markdown")
        return
    else:
        listfed = tl(
            update.effective_message,
...
def __stats__():
    ...
    return tl(
        OWNER_ID,
        "{} pengguna di fbanned, pada {} federasi").format(
        len(all_fbanned),
        len(all_feds))
...
def __user_info__(user_id, chat_id):
    ...
    if fed_id:
        ...
        if int(info['owner']) == user_id:
            text = tl(
                chat_id,
                "Pengguna ini adalah owner di federasi saat ini: <b>{}</b>.").format(infoname)
        elif is_user_fed_admin(fed_id, user_id):
            text = tl(
                chat_id,
                "Pengguna ini adalah admin di federasi saat ini: <b>{}</b>.").format(infoname)
